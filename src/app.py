...
with st.sidebar:
    main_image= st.file_uploader ("Upload Image ", type= ["jpg","png","bmp","jpeg","pgm"], accept_multiple_files=False, key=None, help=None, on_change=None, args=None, kwargs=None, disabled=False, label_visibility="visible")
        ...
        if main_image:
            ...
            with col3:    
                ...
                if(btn1=='Uniform'):
                    ...
                    #st.image(img_noised, caption='noised image' ,width=300,clamp=True) 
                    st.image("../images/noisy.bmp", caption='noised image' ,width=300)
            ...
            with col4:
                if(btn1=='Uniform'): 
                    ...
                    noise=cv2.imread("../images/noisy.bmp",cv2.IMREAD_GRAYSCALE)
                elif(btn1=='salt & pepper'):
                    noise = img_noised
        ...
        with col2:
            if(main_image):
                ...
                if(btn4=='Normalization'):
                    normalize(img=im2)
                    st.image("../images/normalized.bmp", caption='Normalized image' ,width=300)    
...
with tab10:
    ...
    with st.container():
        ...
        if image_for_detect:
            ...
            imge=cv2.imread(path_img,cv2.COLOR_BGR2RGB) 
            imge= cv2.imread(path_img, cv2.IMREAD_GRAYSCALE)
...
with tab11:
    ...
    with st.container():
        col1,col2 = st.columns([2,2])
        if image_recognize:
            path__Rec_img = "../dataset/testing/s41/" + image_recognize.name
            img_Rec=cv2.imread(path__Rec_img,cv2.COLOR_BGR2RGB) 
            img_Rec= cv2.imread(path__Rec_img, cv2.IMREAD_GRAYSCALE)
    ...
    with st.container():
        if(image_recognize):
            col1,a,col2,b= st.columns([2,0.2,2,5])
        ...
        with st.container():
            if(image_recognize):
                ...
                st.image("../images/ROC_CURVE.png", caption='ROC Curve' ,width=600)